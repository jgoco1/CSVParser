    ...
    with open(file_path, 'r', newline='') as infile, open(output_file, 'w', newline='') as outfile:
        ...
        for i, row in enumerate(reader):
            if i >= row_to_start_delete - 1:
                if row[column_to_check].strip() not in values_to_remove:
                    writer.writerow(row)
                else:
                    print("Removing row with value:", row[column_to_check])
            else:
                writer.writerow(row)

#Configuration
row_to_start_codes = 3  # Which row do the codes start in?
column_to_collect_codes = 3 # Which column should codes be collected from?
row_to_start_delete = 2  # Which row is the first to delete?
column_to_check = 9  # Which column should be checked?
...
values_to_remove = read_values_to_remove(values_file, column_to_collect_codes+1, row_to_start_codes)
remove_rows_with_values(file_path, values_to_remove, output_file, column_to_check, row_to_start_delete-1)
